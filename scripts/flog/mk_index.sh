...
#@VERSION:0
#@REVISION:3
#@MACRO:filelog-rebuild|Rebuild the filelog based on filesystem info:{m:exe} noop --preexec `gl_root={m:glroot};{m:exe} -x {siteroot} -R --preexec "echo \"CREATE TABLE filelog(ID INTEGER PRIMARY KEY, path TEXT, size INTEGER, time DATETIME, uid INTEGER, gid INTEGER, status INTEGER);\"" lom "mode!=8" or regex basepath,"^\." -printf "INSERT INTO filelog values(NULL,'{path}',{size},{mtime},{uid},{gid},0);" | sqlite3 /tmp/glutil.frb.$$.tmp; mv /tmp/glutil.frb.$$.tmp {m:q:altlog@file}; chmod 666 {m:q:altlog@file}`
