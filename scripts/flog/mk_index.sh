...
#@VERSION:0
#@REVISION:7
#@MACRO:filelog-rebuild|Rebuild the filelog based on filesystem info:{exe} -noop --preexec `gl_root=\{siteroot\};{exe} -x \{siteroot\} -R --preexec "echo \"CREATE TABLE filelog(ID INTEGER PRIMARY KEY, path TEXT, size INTEGER, time DATETIME, uid INTEGER, gid INTEGER, status INTEGER);\"" -lom "mode!=8" or -l: "basepath" ! -regex "^\." -printf "INSERT INTO filelog values(NULL,\"\{?rd:path:^$gl_root\}\",\{size\},\{mtime\},\{uid\},\{gid\},0);" | sqlite3 /tmp/glutil.frb.$$.tmp; mv /tmp/glutil.frb.$$.tmp \{?q:altlog@file\}; chmod 666 \{?q:altlog@file\}`
