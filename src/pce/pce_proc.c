...
pce_print_msg (char *input, __g_handle hdl)
{
  ...
  if (-1
      == (g_exech_build_string (hdl->buffer.pos ? hdl->buffer.pos->ptr : NULL,
...
pce_process_execv (__g_handle hdl, char *exec_str, __d_avoid_i ppfe)
{
  ...
  return WEXITSTATUS(
      pce_l_execv (exec_args.exec_v_path, exec_args.argv_c, ppfe));
}
	  ...
	  if (gconf->o_exec_on_lookup_fail == 2)
	      ...
	      else
		{
		  ...
		  if (pid2 < 0)
		    {
		      print_str (
			  "ERROR: pce_do_lookup: can't fork after releasing\n");
		      exit (1);
		    }
		  else if (pid2 > 0)
		    exit (0);
