...
net_baseline_fsproto_xfer_stat_ok (__sock pso, __fs_rh_enc pkt, void *arg)
{
  ...
  print_str ("DEBUG: [%d]: sending file request: %s: [%llu] [%llu]\n",
	     pso->sock, psts->data0, packet->body.offset, packet->body.size);
...
net_baseline_fsproto_proc_notify (__sock pso, __fs_rh_enc packet)
{
  ...
  switch (packet->body.m00_8)
      ...
      if (packet->body.status_flags & F_RQH_OP_FAILED)
	{
	  char err_buf[1024];
	  print_str (
	      "WARNING: [%d]: remote 'stat' failed: [%d] [%s]\n", pso->sock,
	      packet->body.err_code,
	      strerror_r (packet->body.err_code, err_buf, sizeof(err_buf)));
	  pso->flags |= F_OPSOCK_TERM;
	}
  ...
  if (psts->hstat.file_size + psts->hstat.file_offset > psts->hstat.size)
    {
      print_str ("ERROR: [%d]: out of range file request: %s | [%llu] [%llu]\n",
		 pso->sock, st_path, psts->hstat.file_offset,
...
net_fs_compile_breq (int code, unsigned char *data, size_t p_len, void *arg)
{
  if (p_len > USHRT_MAX)
    {
      print_str (
	  "ERROR: net_fs_compile_breq: additional header data too big\n");
      return NULL;
    }
  ...
  memcpy ((void*) (((void*) request) + sizeof(_fs_rh_enc)), (void*) data,
	  p_len);
