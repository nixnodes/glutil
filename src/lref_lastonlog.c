			  ...
			  void *mppd)
{
  ...
  void *ptr = ref_to_val_get_cfgval (
      ((struct lastonlog *) arg)->uname, p_b0,
      DEFPATH_USERS,
  ...
  return ref_to_val_get_cfgval (
      ((struct lastonlog *) arg)->gname, p_b0,
      DEFPATH_GROUPS,
      F_CFGV_BUILD_FULL_STRING | F_CFGV_BUILD_DATA_PATH, output, max_size);
}
			 ...
			 void *mppd)
{
  ...
  if (!strncmp (match, _MC_GLOB_LOGON, 5))
    {
      return as_ref_to_val_lk (match, dt_rval_lastonlog_logon,
			       (__d_drt_h ) mppd, "%d");
    }
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_lastonlog_group,
			       (__d_drt_h ) mppd, "%s");
    }
  else if (!strncmp (match, _MC_LASTONLOG_STATS, 5))
    {
      return as_ref_to_val_lk (match, dt_rval_lastonlog_stats,
			       (__d_drt_h ) mppd, "%s");
    }
