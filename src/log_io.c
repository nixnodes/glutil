      ...
      if (!(gfl0 & F_OPT_STDIN))
	{
	  ...
	  if (!(hdl->flags & F_GH_IO_GZIP) && !(hdl->flags & F_GH_IS_GZIP))
	      ...
	      if (hdl->total_sz % hdl->block_sz)
		{
		  print_str (
		  MSG_GEN_DFCORRUW,
			     file, (ulint64_t) hdl->total_sz, hdl->block_sz);
		}
	    }
  ...
  if ((r = load_data_md (&hdl->buffer, hdl->file, hdl)))
    {
	  ...
	  hdl->file,
	  (ulint64_t) hdl->buffer.count,
	  (ulint64_t) (hdl->total_sz / hdl->block_sz),
	  hdl->total_sz,
	  hdl->block_sz,
	  (hdl->flags & F_GH_SHM) ? " [shared memory segment]" : "",
	  r,
	  hdl->hd_errno,
	  hdl->h_errno_gz ? hdl->h_errstr_gz : "GZNOSYSERR",
	  errno ?
	      g_strerr_r (errno, hdl->strerr_b, sizeof(hdl->strerr_b)) :
	      "NOSYSERR");
  ...
  if (!(gfl & F_OPT_NOWRITE))
      ...
      else
	{
	  if (strncmp (hdl->s_buffer, "/tmp/", 5))
		...
		{
		  print_str (
		      "ERROR: %s: [%s] renaming temporary file failed!\n",
		      hdl->s_buffer,
		      g_strerr_r (errno, hdl->strerr_b, sizeof(hdl->strerr_b)));
		  ret = 4;
		}
		...
		{
		  print_str (
		      "ERROR: %s: [%d] [%s] copying temp file failed!\n",
		      hdl->s_buffer, r,
		      g_strerr_r (errno, hdl->strerr_b, sizeof(hdl->strerr_b)));
		  ret = 21;
		}
