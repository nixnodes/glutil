		       ...
		       void *mppd)
{
  ...
  if (!strncmp (match, _MC_GLOB_SIZE, 4))
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_nukelog_unnuker,
			       (__d_drt_h ) mppd, "%s");
    }
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_x_nukelog, (__d_drt_h ) mppd,
			       "%s");
    }
...
char *
dt_rval_nukelog_basedir_e (void *arg, char *match, char *output,
...
nukelog_format_block_batch (void *iarg, char *output)
{
  ...
  return printf ("NUKELOG\x9%s\x9%s\x9%hu\x9%.2f\x9%s\x9%s\x9%s\x9%d\x9%hu\n",
		 ...
		 data->nuker, data->unnuker, data->nukee,
