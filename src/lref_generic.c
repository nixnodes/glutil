...
char *
dt_rval_generic_siterootn (void *arg, char *match, char *output,
...
static char *
dt_rval_generic_null (void *arg, char *match, char *output, size_t max_size,
		      void *mppd)
{
  return "";
}
...
static void *
rt_af_gen_lav (void *arg, char *match, char *output, size_t max_size,
	       void *mppd)
{
		       ...
		       void *mppd)
{
  ...
  if (!strncmp (match, "nukestr", 7))
    {
      return as_ref_to_val_lk (match, dt_rval_generic_nukestr,
			       (__d_drt_h ) mppd, "%s");
    }
  else if (!strncmp (match, "procid", 6))
    {
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_generic_logroot,
			       (__d_drt_h ) mppd, "%s");
    }
  else if (!strncmp (match, "memlimit", 8))
    {
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_generic_curtime,
			       (__d_drt_h ) mppd, "%d");
    }
  else if (!strncmp (match, "q:", 2))
    {
  ...
  else if (!strncmp (match, _MC_GLOB_S64G, 7))
    {
      ...
      return as_ref_to_val_lk (match, dt_rval_gg_sint, (__d_drt_h ) mppd,
			       "%lld");
    }
  else if (!strncmp (match, _MC_GLOB_F32G, 7))
    {
	...
	case 0x6E:
	  return as_ref_to_val_lk (match, dt_rval_generic_newline,
				   (__d_drt_h ) mppd, "%s");
	case 0x74:
	  return as_ref_to_val_lk (match, dt_rval_generic_tab,
				   (__d_drt_h ) mppd, "%s");
	case 0x72:
    ...
    {
      return as_ref_to_val_lk (match, dt_rval_generic_ftpdata,
			       (__d_drt_h ) mppd, "%s");
    }
  else if (!strncmp (match, "logfile", 7))
    {
      return as_ref_to_val_lk (match, dt_rval_generic_logfile,
			       (__d_drt_h ) mppd, "%s");
    }
  else if (!strncmp (match, "imdbfile", 8))
    {
  ...
  else if (!strncmp (match, "null", 4))
    {
      return as_ref_to_val_lk (match, dt_rval_generic_null, (__d_drt_h ) mppd,
      NULL);
    }
